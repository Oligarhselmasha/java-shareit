...
CREATE TABLE IF NOT EXISTS REQUESTS_ITEMS ( id INTEGER GENERATED ALWAYS AS IDENTITY PRIMARY KEY, request_id INTEGER, items_id INTEGER,
CONSTRAINT fk_requests_line_to_requests FOREIGN KEY(request_id) REFERENCES requests(id), CONSTRAINT fk_requests_line_items FOREIGN KEY(items_id) REFERENCES items(id));